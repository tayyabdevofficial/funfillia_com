...
@section('title', ($result['friend_name'] ?? 'You') . ' scored ' . ($result['percentage'] ?? 0) . '% on ' . ($result['creator_name'] ?? 'Friend') . "'s Dare! - Funfillia")
@section('meta_description', 'Check out the quiz results and see who knows who best!')
...
<div class="min-h-screen bg-slate-50 dark:bg-slate-950 py-8 sm:py-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        @php
            ...
            $verdict = $result['verdict'] ?? 'Best Friends';
            $shareUrl = route('quizzes.challenge.take', $token);

            $boastText = rawurlencode(
                "🎉 *BOAST TIME! I CRUSHED THE DARE!* 🎉\n\n" .
                "⚡ I just scored *{$score}/{$total} ({$percentage}% Match)* on *{$creatorName}*'s Friendship Dare!\n\n" .
                "🏅 *Our Verdict:* {$verdict} ✨\n\n" .
                "😏 *Think you know {$creatorName} better than me?*\n" .
                "👇 *Accept the dare and try to beat my score:* 👇\n" .
                "{$shareUrl}\n\n" .
                "🔥 Take the challenge now & let's see who tops the Leaderboard! 👑🚀"
            );
        @endphp

        <!-- Result Score Card -->
        <div class="bg-gradient-to-br from-purple-600 via-pink-600 to-rose-500 rounded-3xl text-white p-6 sm:p-10 shadow-2xl text-center relative overflow-hidden">
            ...
            <div class="relative z-10 space-y-5">
                <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 backdrop-blur-md text-xs font-black uppercase tracking-wider text-yellow-300 shadow-sm">
                    <span>✨</span> Dare Challenge Completed!
                </div>
                ...
                <div class="py-4">
                    <div class="inline-flex flex-col items-center justify-center w-36 h-36 sm:w-44 sm:h-44 rounded-full bg-white/15 backdrop-blur-xl border-4 border-white/40 shadow-inner">
                        ...
                        <span class="text-sm sm:text-base font-extrabold text-yellow-300">
                            {{ $percentage }}% Match
                        </span>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-sm space-y-4 text-center">
            <h2 class="text-lg font-black text-slate-900 dark:text-white">
                Boast Your Score & Dare Other Friends!
            </h2>
            <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                Share how well you scored on WhatsApp and see if any mutual friends can beat you.
            </p>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                ...
                <a href="{{ route('quizzes.index') }}" 
                   class="w-full sm:w-auto px-6 py-3.5 rounded-2xl bg-gradient-to-r from-pink-500 to-purple-600 text-white font-black text-sm shadow-md hover:shadow-lg hover:scale-105 transition-all flex items-center justify-center gap-2">
                ...
                @foreach($result['comparison'] ?? [] as $idx => $comp)
                    <div class="p-5 rounded-3xl border-2 {{ $comp['is_match'] ? 'border-emerald-500/40 bg-emerald-50/40 dark:bg-emerald-950/20' : 'border-rose-500/40 bg-rose-50/40 dark:bg-rose-950/20' }} shadow-sm space-y-3">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-xs sm:text-sm">
                            <!-- Friend Choice -->
                            <div class="p-3 rounded-2xl {{ $comp['is_match'] ? 'bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-800' : 'bg-white dark:bg-slate-900 border border-rose-200 dark:border-rose-800' }}">
                            ...
                            <div class="p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                                <div class="text-[10px] font-bold uppercase tracking-wider text-purple-600 dark:text-purple-400 mb-1">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-sm space-y-4">
                ...
                <div class="divide-y divide-slate-100 dark:divide-slate-800">
                    @foreach($result['leaderboard'] as $i => $item)
                        ...
                        @endphp
                        <div class="py-3 flex items-center justify-between gap-4 {{ $isCurrentAttempt ? 'bg-pink-50/50 dark:bg-pink-950/30 px-3 -mx-3 rounded-xl' : '' }}">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <span class="font-bold text-sm text-slate-900 dark:text-white">
                                        {{ $item['friend_name'] }}
                                        @if($isCurrentAttempt)
                                            <span class="ml-1 text-[10px] font-black uppercase text-pink-600 dark:text-pink-400">(You)</span>
                                        @endif
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-black {{ $item['percentage'] >= 70 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300' : 'bg-purple-100 text-purple-700 dark:bg-purple-950/60 dark:text-purple-300' }}">
                                    {{ $item['percentage'] }}%
                                </span>
                            </div>
                        </div>
        ...
        <!-- Huge Bottom Viral Call To Action -->
        <div class="p-8 rounded-3xl bg-gradient-to-r from-pink-500 via-rose-500 to-purple-600 text-white text-center space-y-4 shadow-xl">
            ...
            <p class="text-sm sm:text-base text-white/90 max-w-md mx-auto font-medium">
                Create your own quiz challenge now. Let's see if {{ $result['creator_name'] }} knows you as well as you know them!
            </p>
            <a href="{{ route('quizzes.index') }}" class="inline-flex items-center gap-2 px-8 py-4 rounded-2xl bg-white text-slate-900 font-black text-base shadow-lg hover:bg-yellow-300 hover:scale-105 transition-all">
