...
@section('title', ($challenge['creator_name'] ?? 'Your') . "'s Dare Challenge - Share & Scoreboard - Funfillia")
@section('meta_description', 'Share your dare challenge with your friends on WhatsApp, Messenger, and group chats, and track who gets the highest score!')
...
<div class="min-h-screen bg-slate-50 dark:bg-slate-950 py-8 sm:py-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        @php
            ...
            $quizTitle = $quiz['title'] ?? 'The Friendship Dare Challenge';

            $whatsappMsg = rawurlencode(
                "{$creatorAvatar} *{$creatorName}* has officially challenged YOU to a Friendship Dare!\n\n" .
                "🤔 *Think you REALLY know them?* Let's put your bond to the test! 🤫\n\n" .
                "🎯 *Challenge:* {$quizTitle}\n\n" .
                "👇 *Tap the link below to accept the dare:* 👇\n" .
                "{$shareUrl}\n\n" .
                "⚡ Guess their secret answers and see your rank on the live Leaderboard! 🏆👑"
            );
        @endphp

        <!-- Success Celebration Card -->
        <div class="bg-gradient-to-br from-pink-500 via-rose-500 to-purple-600 rounded-3xl text-white p-6 sm:p-10 shadow-2xl text-center relative overflow-hidden">
            ...
            <div class="relative z-10 space-y-4">
                ...
                <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-black uppercase tracking-wider text-yellow-300">
                    <span>🎉</span> Dare Challenge Created!
                </div>

                <h1 class="text-2xl sm:text-4xl font-black tracking-tight">
                    {{ $challenge['creator_name'] }}'s Friendship Dare
                </h1>

                <p class="text-sm sm:text-base text-white/90 max-w-lg mx-auto">
                    Based on <strong>{{ $quiz['title'] ?? 'Quiz' }}</strong>. Share your secret link below and watch your friends try to guess your choices!
                </p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-sm space-y-6">
            <div class="space-y-1 text-center sm:text-left">
                <h2 class="text-lg font-black text-slate-900 dark:text-white">Share With Your Friends</h2>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Send this link directly to WhatsApp contacts, Discord servers, or group chats.</p>
            </div>

            <!-- WhatsApp One-Click Share (Massive Prominence) -->
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-2 p-2 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
                    ...
                    <button type="button" onclick="copyShareLink()" id="copy-btn"
                            class="px-4 py-2 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold text-xs sm:text-sm hover:bg-pink-600 dark:hover:bg-pink-400 dark:hover:text-white transition-colors shrink-0 flex items-center gap-1.5">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-sm space-y-6">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                ...
                <div class="px-3 py-1 rounded-full bg-pink-100 dark:bg-pink-950/60 text-pink-600 dark:text-pink-400 text-xs font-black">
            ...
            @if(empty($leaderboard) || count($leaderboard) === 0)
                <div class="text-center py-12 space-y-3">
                    ...
                    <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
                        Share your link on WhatsApp or social media above. When they answer, their scores will pop up here instantly!
                    </p>
                </div>
            @else
                    ...
                    @foreach($leaderboard as $idx => $attempt)
                        <div class="py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                ...
                                <div>
                                    <div class="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-2">
                                        ...
                                        @if(!empty($attempt['verdict']))
                                            <span class="text-[11px] font-semibold text-purple-600 dark:text-purple-400">({{ $attempt['verdict'] }})</span>
                                        @endif
                                    </div>
                            ...
                            <div class="flex items-center gap-2.5 self-end sm:self-auto">
                                ...
                                <span class="px-2.5 py-1 rounded-full text-xs font-black {{ $attempt['percentage'] >= 70 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300' : ($attempt['percentage'] >= 50 ? 'bg-blue-100 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300') }}">
                                    {{ $attempt['percentage'] }}%
                                </span>
                                ...
                                @if(!empty($attempt['breakdown']))
                                    <button type="button" 
                                            ...
                                            class="px-3 py-1.5 rounded-xl bg-pink-50 hover:bg-pink-100 dark:bg-pink-950/60 dark:hover:bg-pink-900/80 text-pink-600 dark:text-pink-300 text-xs font-bold inline-flex items-center gap-1.5 transition-colors shadow-sm cursor-pointer">
                                        <span>👁️</span>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4 text-center">
            <a href="{{ route('quizzes.challenge.take', $challenge['share_token']) }}?preview=1" class="text-xs sm:text-sm font-bold text-slate-600 dark:text-slate-400 hover:text-pink-600 dark:hover:text-pink-400 transition-colors">
                👀 Preview Friend View
            </a>
            <span class="hidden sm:inline text-slate-300 dark:text-slate-700">&bull;</span>
            <a href="{{ route('quizzes.index') }}" class="text-xs sm:text-sm font-bold text-slate-600 dark:text-slate-400 hover:text-pink-600 dark:hover:text-pink-400 transition-colors">
                ✨ Create Another Quiz
            </a>
        </div>
...
<div id="answers-modal" 
     class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-200"
     onclick="if(event.target === this) closeAnswersModal()">
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-xl max-h-[85vh] flex flex-col transform scale-95 transition-all duration-200 overflow-hidden" id="answers-modal-box">
        <!-- Modal Header -->
        <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-pink-50/50 via-purple-50/30 to-white dark:from-slate-800/80 dark:to-slate-900">
            <div>
                <div class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-pink-100 dark:bg-pink-950 text-pink-600 dark:text-pink-400 text-[10px] font-black uppercase tracking-wider mb-1">
        ...
        <!-- Modal Footer -->
        <div class="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-850 flex justify-end">
...
<script>
    ...
    window.openAnswersModal = function(idx) {
        ...
        if (attempt.breakdown && attempt.breakdown.length > 0) {
            attempt.breakdown.forEach((item, qIdx) => {
                ...
                truthCol.innerHTML = `
                    <span class="text-[10px] font-bold text-pink-600 dark:text-pink-400 uppercase tracking-wider block">Your Truth</span>
                    <div class="font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-1.5 mt-0.5">
                        <span>✨</span>
    ...
    // Save this dare challenge into localStorage for user history
    try {
        const currentDare = {
            token: @json($challenge['share_token'] ?? ''),
            quiz_title: @json($quiz['title'] ?? 'Quiz Dare'),
            creator_name: @json($challenge['creator_name'] ?? 'You'),
            creator_avatar: @json($challenge['creator_avatar'] ?? '😎'),
            created_at: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
        };
        let stored = JSON.parse(localStorage.getItem('funfillia_my_dares') || '[]');
        if (!stored.some(d => d.token === currentDare.token)) {
            stored.unshift(currentDare);
            localStorage.setItem('funfillia_my_dares', JSON.stringify(stored.slice(0, 20)));
        }
    } catch(e) {}
</script>
@endsection
